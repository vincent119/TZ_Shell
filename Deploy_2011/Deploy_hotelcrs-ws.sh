...
################################################
################################################
BIN_FILE="hotelcrs-ws-project-*-binary.zip "
################################################
########### defind variables for prod ##########
if [ $SER_TYPE = "prod" ];then
  DPATH="/opt/deploy/$DDDATE"
  CON_FILE="hotelcrs-ws-project-*-prod-config.zip"  

########## defind variables for beta ###########
elif [ $SER_TYPE = "beta" ];then
  CON_FILE="hotelcrs-ws-project-*-shbeta-config.zi"   
  DPATH="/opt/Deploy_Beta/Deploy_Beta/$DDDATE"

######## defind variables for alpha ############
elif [ $SER_TYPE = "alpha" ];then
  CON_FILE="hotelcrs-ws-project-*-alpha-config.zip"
  DPATH="/opt/deploy/$DDDATE"
fi
#########################################
SERVICE_N="hotelcrs-ws"
ENV_NAME="JBOSS-HOTEL"
################################################
