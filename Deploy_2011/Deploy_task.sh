...
################################################
################################################
BIN_FILE="ota-web-service.war"
################################################
########### defind variables for prod ##########
if [ $SER_TYPE = "prod" ];then
  DPATH="/opt/deploy/$DDDATE"
  if [ $SITE = "hk" ];then
    CON_FILE="tz-jboss-env-*-hktask-config.zip"
  elif [ $SITE = "sh" ];then 
    CON_FILE="tz-jboss-env-*-shtask-config.zip" 
  fi
fi
#########################################
SERVICE_N="tz-task"
...
if [ ! -s $DPATH/$DDDATE/$ENV_NAME/$CON_FILE ];then
  ...
  if [ -s $DPATH/$DDDATE/$ENV_NAME/deploy/$BIN_FILE ];then
     rm -rf $JBOSSHOME/$SERVICE_N/deploy/$BIN_FILE
     cp $DPATH/$DDDATE/$ENV_NAME/$BIN_FILE $JBOSSHOME/$SERVICE_N/deploy/
  fi
  if [ -s $DPATH/$DDDATE/$ENV_NAME/$CON_FILE ];then
